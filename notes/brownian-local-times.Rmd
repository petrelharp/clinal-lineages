...
\begin{aligned}
    p(x,r,\tau,A) = 
    \frac{1}{2} \left\{ 1 - \sgn(x) \zeta(x,r,\tau) p_H \right\} .
\end{aligned}
$$

Does this actually make a cline?
```{r show_cline}
pcline <- function (x,r,tau=1,sigma=1,pH=1/2) {
    (1/2)*( 1 - sign(x)*zeta(x=x/sqrt(tau*sigma^2),r=r*tau)*pH )
}
xx <- seq(-15,15,length.out=500)
for (tau in c(5,50)) {
    plot(xx, pcline(xx,r=1), type='n', 
            xlab="distance from cline center", ylab="prob of inherited from the left", 
