...
rpairs <- as.matrix(expand.grid(r1=rr,r2=rr))
LD.solns <- parallel::mclapply( 1:nrow(rpairs), function (k) forwards_backwards_haplotypes(s=s, times=tt, xgrid=xgrid, rgrid=list(x.mid=rpairs[k,],N=2), fwds.soln=fwds.soln), mc.cores=parallel::detectCores() )
```

Here are heatmaps of ancestry LD at a few different times.
The selected site is in the center.
```{r tens_ld_heatmaps, cache=TRUE, depends="tens_ld_solutions"}
```
