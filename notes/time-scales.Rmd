...
```{r tens_fwds_soln_plot, cache=TRUE, depends="setup_tens"}
...
Here's the behaviour of the selected site that we use to describe the behaviour of the other sites:
```{r hunds_fwds_soln_plot, cache=TRUE, depends="setup_hunds"}
image(fwds.soln, legend=TRUE, zlim=c(0,1), yaxt='n', main="selected site", ylab="space")
add_grid_axis(xgrid)
```
...
```{r hunds_linked_clines, cache=TRUE, depends="setup_hunds"}
linked.solns <- lapply( rr, function (r) {
            forwards_backwards_pde(s=s,times=tt,grid=xgrid,r=r,log.p=TRUE,fwds.soln=fwds.soln)
        } )
linked.clines <- lapply( linked.solns, cline_from_soln, grid=xgrid )
...
Here are linked clines for a few time steps:
```{r plot_hunds_linked_clines, cache=TRUE, depends="hunds_linked_clines"}
...
Here are heatmaps of ancestry LD at a few different times and locations
The selected site is in the center.
```{r hunds_ld_heatmaps, cache=TRUE, depends="hunds_ld_solutions", fig.width=3*fig.dim}
plot.x <- c(0,5,10)
layout(t(1:3))
for (plot.t in 10*c(4,10,20,40)) {
    for (k in seq_along(plot.x)) {
...
Here's the behaviour of the selected site that we use to describe the behaviour of the other sites:
```{r thous_fwds_soln_plot, cache=TRUE, depends="setup_thous"}
image(fwds.soln, legend=TRUE, zlim=c(0,1), yaxt='n', main="selected site", ylab="space")
add_grid_axis(xgrid)
```
...
```{r thous_linked_clines, cache=TRUE, depends="setup_thous"}
linked.solns <- lapply( rr, function (r) {
            forwards_backwards_pde(s=s,times=tt,grid=xgrid,r=r,log.p=TRUE,fwds.soln=fwds.soln)
        } )
linked.clines <- lapply( linked.solns, cline_from_soln, grid=xgrid )
...
Here are linked clines for a few time steps:
```{r plot_thous_linked_clines, cache=TRUE, depends="thous_linked_clines"}
...
Here are heatmaps of ancestry LD at a few different times and locations
The selected site is in the center.
```{r thous_ld_heatmaps, cache=TRUE, depends="thous_ld_solutions", fig.width=3*fig.dim}
plot.x <- c(0,5,10)
layout(t(1:3))
for (plot.t in 100*c(4,10,20,40)) {
    for (k in seq_along(plot.x)) {
