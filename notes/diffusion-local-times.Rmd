...
```{r fwds_back_pde, cache=TRUE, depends="fwds_pde"}
yinit <- rep(1.0,thegrid$N)
rev.pde <- function (t,y,parms,...) {
    p <- cline.interp(tau-t,soln=fwds.soln)
    # p <- setup.prop.1D(grid=thegrid, xy=cbind(thegrid$x.mid,cline.interp(tau-t)), interp="linear")
    tran <- tran.1D(C=y, A=p, D=1/2, dx=thegrid)$dC
    list( tran - r * (1-p) * y )
}
...
```{r compare_rightness, cache=TRUE,depends=c("fwds_back_pde","solve_pde_setup","setup_fns")}
layout(t(1:2))
...
clineplot(pde.soln.0,x=attr(pde.soln.0,"x"),times=seq_along(tt),xlim=c(-5,0))
...
```{r sim_clines, cache=TRUE, depends=c("sim_pushed","pushed_pde")}
...
And, finally, here's some actual clines predicted by this model:
```{r pushed_cline,cache=TRUE,depends="pushed_pde"}
tau <- 100
tt <- seq(0,tau,length.out=100)
pde.solns <- lapply( rvals, function (r) {
        pde.fn <- function (t,y,parms,...) {
...
plot.tvals <- floor(seq(0,tau,length.out=7))[-1]
layout(t(1:2))
for (tval in plot.tvals) {
    matplot( thegrid$x.mid, 
            pcline(thegrid$x.mid, p=sapply( pde.solns, cline.interp, T=tval ) ), 
                   type='l', main=sprintf("t=%d",tval) 
        )
    legend("bottomleft",lty=1:5,col=1:6,legend=paste("r=",round(rvals/2,digits=2)))
}
```
