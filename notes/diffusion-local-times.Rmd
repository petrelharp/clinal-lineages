...
$$\begin{aligned}
    \frac{d}{dt} \zeta(x,t,r) 
    &= 
    \frac{\sigma^2}{2} \frac{d^2}{dx^2} \zeta(x,t,r) + \frac{d}{dx} \log(p(x)) \frac{d}{dx} \zeta(x,t,r)  - r f_H(x) \zeta(x,t,r)  \\
    \zeta(x,0,r) = 0 .
\end{aligned}$$
...
```{r solve_pde}
thegrid <- setup.grid.1D(x.up=-10, x.down=10, N = 1000)
driftfn <- setup.prop.1D(grid=thegrid, func=tanh_drift, s=s)
...
```{r sim_general_longer, fig.height=3*fig.dim, cache=TRUE}
nwalks <- 1e3
ntimes <- 1e4
s <- 0.1
tau <- 100
