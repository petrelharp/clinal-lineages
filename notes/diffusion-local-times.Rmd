...
rws <- rbind(0,matrix( rnorm(nwalks*(ntimes-1))*sqrt(dt), nrow=(ntimes-1), ncol=nwalks ))
for (k in 2:ntimes) {
    rws[k,] <- rws[k-1,]+rws[k,]+dt*tanh_drift(rws[k-1,],s)
}
...
lts <- dt*apply( matrix(findInterval(rws,xx),nrow=nrow(rws)), 2, tabulate, nbins=length(xx)-1 )
layout( t(1:2), widths=c(3,1) )
...
<<<<<<< HEAD:notes/brownian-local-times.Rmd
...
```{r show_cline}
pcline
>>>>>>> github/master:notes/diffusion-local-times.Rmd
xx <- seq(-15,15,length.out=500)
for (tau in c(5,50)) {
...
$$
Here's what that looks like:
```{r tanh_cline, cache=TRUE}
tanh_cline <- function (x,s,sigma=1) {
    (1/2)*(1+tanh(-2*x*sqrt(s)/sigma))
}
fH <- function (x,s,pfn=tanh_cline,...) {
    px <- pfn(x,s=s,...)
    ( (1-s)*(1-px) )/( 1 - s*(1-px) )
}
plot(xx, tanh_cline(xx,s=0.04), type='l' )
```
...
```{r sim_lineage, cache=TRUE, depends=c("tanh_cline","show_cline","killing")}
...
total.time <- 10
s <- 0.1
dt <- total.time/ntimes
# put the noise in rws
rws <- rbind( rnorm(nwalks), # initial conditions
             matrix( rnorm(nwalks*(ntimes-1)), nrow=(ntimes-1), ncol=nwalks ) )
# and then integrate over it
for (k in 2:ntimes) {
    rws[k,] <- rws[k-1,]+sqrt(dt)*rws[k,]+dt*drift(rws[k-1,],s)
}
layout( t(1:2), widths=c(3,1) )
matplot(seq(0,total.time,length.out=ntimes),rws[,1:80],type='l', ylab='position', xlab='time', ylim=range(rws) )
legend("topright",lwd=2,legend=c("prob of het","cline","drift"),col=c("black","green","red"))
plot( (-1)*drift(xx,s=s), xx, col='red', lwd=2, type='l', ylim=range(rws), ylab='', xlab='drift or prob' )
...
Let's just check that's doing the right thing:
```{r sim_lineage_check, cache=TRUE, depends="sim_lineage"}
xbreaks <- seq(-3,3,length.out=100)
xmids <- xbreaks[-1]-diff(xbreaks)/2
dX <- apply(rws,2,diff)
Xmeans <- tapply( dX, cut(rws[-nrow(rws),],breaks=xbreaks), mean )
Xvar <- tapply( dX, cut(rws[-nrow(rws),],breaks=xbreaks), var )
layout(1:2)
plot(xmids,Xmeans/dt,ylab="mean dX")
lines(xmids, drift(xmids,s), col='red', lwd=2)
plot(xmids,Xvar/dt,ylab="variance dX")
abline(h=1,col='red',lwd=2)
```

**Comparison to zero-width cline:**
Let's start a bunch of random walks sorta far from the cline center,
and look at the distribution of the time spend in heterozygotes.
```{r sim_het_time, cache=TRUE, depends="sim_lineage"}
nwalks <- 1e4
ntimes <- 3e3
total.time <- 100
s <- 0.1
dt <- total.time/ntimes
# put the noise in rws
rws <- rbind( -10, # initial conditions
             matrix( rnorm(nwalks*(ntimes-1)), nrow=(ntimes-1), ncol=nwalks ) )
# and then integrate over it
for (k in 2:ntimes) {
    rws[k,] <- rws[k-1,]+sqrt(dt)*rws[k,]+dt*drift(rws[k-1,],s)
...
lines( (1-s)*(1-tanh_cline(xx,s=s))/(1-s*(1-tanh_cline(xx,s=s))), xx, lwd=2 )
```
Here is the distribution of $T=\int_0^{`r total.time`} f_H(X_t) dt$:
```{r dist_het_time, cache=TRUE, depends="sim_het_time"}
het.time <- dt*colSums( fH(rws,s=s) )
layout(1:2)
