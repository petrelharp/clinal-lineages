...
```{r setup_fns}
pcline <- function (x,r,recombfn,...) {
    (1/2)*( 1 - sign(x)*recombfn(x,2*r,...) )
}
...
clineplot <- function (soln,x=thegrid$x.mid,times=seq(1,nrow(soln),length.out=ntimes),ntimes=20,ylab="probability",ylim=c(0,1),...) {
    ...
    legend("bottomleft", lty=1, col=rainbow(length(times)), legend=paste("t=",round(soln[times,1],digits=2)), cex=0.5)
...
$ \begin{align*}
  \partial_t p(x,t) = \frac{\sigma^2}{2} \partial_x^2 + sp(x,t)(1-p(x,t))(2p(x,t)-1) .
\end{align*} $
...
yinit <- rep(1.0,thegrid$N)
cline.interp <- function (T) {
    # return the interpolated cline for a particular time
    which.t <- findInterval(T,fwds.soln[,1],rightmost.closed=TRUE)
    lu <- fwds.soln[c(which.t,which.t+1),1]
    alpha <- (T-lu[1])/diff(lu)
    return( (1-alpha)*fwds.soln[which.t,-1] + alpha*fwds.soln[which.t+1,-1]  )
}
rev.pde <- function (t,y,parms,...) {
    p <- cline.interp(tau-t)
...
zeta.pcline <- function (x,r,recombfn=function(x,r){zeta.reflected(x=x/sqrt(tau*sigma^2),r=r*tau)},tau=1,sigma=1) {
    (1/2)*( 1 - sign(x)*zeta.reflected(x=x/sqrt(tau*sigma^2),r=r*tau) )
}
xx <- seq(-15,15,length.out=500)
for (tau in c(5,50)) {
