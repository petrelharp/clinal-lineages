...
cline.A <- setup.prop.1D(grid=thegrid, func=tanh_cline, s=s)
cline.B <- setup.prop.1D(grid=thegrid, func=function(x,s)1-tanh_cline(x,s), s=s)
...
pde.fn <- function (t,y,parms,...) {
    ...
    list( c( 
            tran.A + r * cline.B$mid * (yB-yA),
            tran.B + r * cline.A$mid * (yA-yB)
            ) )
}
pde.soln <- ode.1D( y=yinit, times=tt, func=pde.fn, nspec=2 ) # note FIRST COLUMN IS TIME
