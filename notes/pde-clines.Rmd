...
solve.pde <- function ( u, v=u, r, times, grid, ... ) {
    ...
    cline.A <- setup.prop.1D(grid=grid, func=u, ...)
    cline.B <- setup.prop.1D(grid=grid, func=function(x,...)1-u(x,...), ...)
    yinit <- c( rep(1.0,grid$N), rep(0.0,grid$N) )
    r <- 0.1
    pde.fn <- function (t,y,parms,...) {
        yA <- y[1:grid$N]
        yB <- y[grid$N+(1:grid$N)]
        tran.A <- tran.1D(C=yA, A=cline.A, D=1/2, dx=grid)$dC
        tran.B <- tran.1D(C=yB, A=cline.B, D=1/2, dx=grid)$dC
        list( c( 
                tran.A + r * (1-vfn(x=grid$x.mid,t=t)) * (yB-yA),
                tran.B + r * vfn(x=grid$x.mid,t=t) * (yA-yB)
                ) )
    }
...
cline.from.soln <- function (soln, grid, clinefn=attr(soln,"u"), ... ) {
    ...
    sel.cline <- clinefn(grid$x.mid,s=s)
...
s <- 0.1
zone.age <- 100  # this is tau
tt <- seq(0,zone.age,length.out=100)
r <- 0.1
pde.soln <- solve.pde( u=tanh_cline, r=r, times=tt, s=s, grid=thegrid )
...
```{r match_pde}
tt <- seq(0,zone.age,length.out=100)
pde.solns <- lapply(rvals, function (r) { solve.pde( u=tanh_cline, r=r, times=tt, s=s ) })
```
