...
pde.soln <- solve.pde( u=log_u, um1=log_1mu, r=r, times=tt, s=s, grid=thegrid, log.u=TRUE )
...
```{r match_pde}
tt <- seq(0,zone.age,length.out=100)
pde.solns <- lapply(rvals, function (r) { solve.pde( u=function(...)1-tanh_cline(...), r=r, times=tt, s=s, grid=thegrid ) })
pde.clines <- lapply( pde.solns, cline.from.soln, grid=thegrid )
match.clines <- sapply(pde.clines,function(x)x[length(tt),])
```
...
for (k in seq_along(rvals)) {
    ...
    lines(xx,1-tanh_cline(xx,s=s,sigma=SIGMA),col='red',lwd=3)
    lines(1-thegrid$x.mid*SIGMA,match.clines[,k],col='green',lwd=3)
}
```
