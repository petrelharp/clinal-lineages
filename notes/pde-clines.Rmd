...
Ig_A(a,b)
    ...
    &= 
    p \int_a^b
        \left( g_A(a,\theta) g_A(\theta,b)  - g_A(a,b) \right) d\theta 
    \\ &\qquad {}
    + (1-p) \int_a^b \one_{\theta>0} \left( g_A(a,\theta) g_B(\theta,b) - g_A(a,b) \right) d\theta
    \\ &\qquad {}
    + (1-p) \int_a^b \one_{\theta\le0} \left( g_B(a,\theta) g_A(\theta,b) - g_A(a,b) \right) d\theta  .
\end{aligned}$$
...
recomb_generator <- function (x,eps) {
    # the discretization of the map y(a,b) -> \int_a^b (h(a,u)*h(u,b)-h(a,b)) du
    # x is the upper triangular part of a square matrix
    #   eps is the grid spacing between locations
    x2 <- eps*unifprod_ut(x)
}
xgrid <- setup.grid.1D(x.up=-20, x.down=20, N=10)
agrid <- setup.grid.1D(x.up=0.0, x.down=0.5, N=5)
# function to find a,b coordinates from the upper triangular coordinates
ab.ord <- matrix( 0, nrow=agrid$N, ncol=agrid$N )
avals <- agrid$x.mid[row(ab.ord)[upper.tri(ab.ord,diag=TRUE)]]
bvals <- agrid$x.mid[col(ab.ord)[upper.tri(ab.ord,diag=TRUE)]]
rvals <- ifelse( avals*bvals<0, 0, pmin(avals,bvals) )
...
soln.array <- matrix(0.0,nrow=2*xgrid$N,ncol=agrid$N*(agrid$N+1)/2)
    ...
    rev.pde.fn <- function (t,y,parms,...) {
        ...
        recombination <- apply( y, 1, recomb_generator, p=p )
        return( list( diffusion + recombination ) )
    }
    rev.soln <- ode.1D( y=yinit, times=times, func=rev.pde.fn, nspec=2*ncol(soln.array), tcrit=max(fwds.soln[,1]) )
