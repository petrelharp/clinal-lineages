...
Our findings suggest that the density of ancestry junctions provide a means for characterizing hybrid zones and identifying candidate targets of selection.
...
We study those maintained only by selection against hybrids -- \emph{tension zones} --
...
and \citet{barton1986barrier} show that the shapes of several sorts of stable clines are similar.
...
the two alleles at the selected locus will form a stable cline \citep{barton1979gene},
...
(iii) $s$ and $\sigma$ are small, 
and (iv) population density is large,
then the commonly-used equation that $p$ solves is
...
as in \citet{Bazykin1969}; 
this can be derived as in \citet{Nagylaki1975};
for discussion of diploidy see \citet{christiansen1995genotypic}.
As we show below the solutions provide an good approximation across the range of realistic values of $s$.
...
in heterozygotes the lineage linked to the selected locus may move between backgrounds. 
...
(see Appendix \ref{apx:lineage_derivation}).

We can describe this process formally in It\^o notation:
with $B_\tau$ a standard Brownian motion,
$T_B$ the most recent time that $Z_\tau = B$, and likewise for $T_A$,
\begin{align}
...
We now find the frequency at which \emph{entire} blocks of genome (haplotype) 
% YB: thought `seeing' was too much about our inference rather than the the biological process
% PLR: hm, it's not 'maintaining' either, since the block might have been broken up along the way.
...
sampled at spatial location $x$ at time $t$ after the initiation of gene flow. % I think people sometimes use 'cline formed' to mean when the equilibrium was established, changed for clarity
...
In this description,  % don't like to have math at the start of a sentence
...
Both factors are expected to result in regions that are surrounded by even longer segments of unbroken ancestry, and will impact expectations of genome-wide pattens of clines and block lengths, as well as the resolution to which one could detect targets of selection. 
% \yb{provide citation.} % grr, can't find it.
