...
\plr{say what population density in simulation figures}
...
\plr{what is the sum over? define $m_i$.}

We are interested in identifying regions of the genome with abnormal distributions of block lengths, and where indicated normalize $l_I(m)$ and $C(m)$ by dividing respectively by \plr{the empirical} mean \plr{of} $l_I$ and $C$  within \plr{the corresponding} deme. 
\plr{which deme? should say that this is computed separately by deme?}
While there is useful information contained in the geographic distribution of block length patterns, obtaining good spatial sampling can often be difficult, and this allows us to predict patterns within relatively few populations. 
