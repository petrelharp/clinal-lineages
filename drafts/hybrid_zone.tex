...
to better understand the mechanisms underlying reproductive isolation and the process of speciation. 
Here, we use a combination of analytical theory and explicit forward simulations
to gain a better understanding of how selection against hybrid gentypes impacts patterns of introgression 
...
whose lineages trace back to the side they originated on at speed $\sqrt{s}$ dispersal distances per generation.
...
can be labeled according to which ancestral population is is inherited from,
and local interbreeding and migration of such populations
creates a gradient of ancestry assignments across geographic space,
...
forming chromosomal \emph{junctions} in ancestry \citep{Fisher1954, Chapman2002, Baird2003},
leading to substantial correlations in ancestry (and hence, genotype) between distant markers.
...
The arrangement of segments of genome inheriting from particular ancestral populations
provides information about the number of recombination events that have taken place between ancestral genotypes, 
and therefore provides information about the age of the hybrid zone and the relative strength of selection affecting subsets of the genome. 
...
If hybrids have non-zero fitness, some amount of introgression can occur, providing information about the extent of hybridization and timing of secondary contact \citep{Price2009, Hellenthal2014}. 
The resulting heterogeneous patterns of ancestry across the genome have been used to identify putative targets of selection in hybridizing populations,
by examining heterogeneous clines in ancestry across loci \citep{Porter1997, Gompert2012}.
...
In \citet{Sedghifar2015} we described how lineages move across a neutral zone of secondary contact,
...
The mean haplotype length found without conditioning on ancestry 
shows a smaller increase near the selected locus,
shown in Figures \ref{Fig:meanBlockLengthsUnconditioned}, \ref{Supp:blockLengthHeatmapNoAnc} and \ref{Supp:blockLengthNoAnc}.

\alisa{
For weaker selection ($s=0.001$), then the relative speed of retreat is not fast enough compared to the genomic background, and the strength of signal from elevated $l_B(m)$ is diminished. The resolution of the peak is expected to scale with $1/T$, as this is the physical scale over which linked clines persist. Since clines in ancestry establish over the timescale $1/s$, it is difficult to intuit the time it takes for a signal of elevated $l_B(m)$ to form around the selected locus. 
...
\yaniv{remove this? or, what figure?}
