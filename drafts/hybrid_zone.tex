...
blocks of ancestry surrounding incompatibilities can be substantially longer than the genome-wide average block length at the same spatial location, %single-locus incompatibilities can be substantially longer than the genome-wide average block length at the same spatial location, % YB Comment. I think single-locus is a red-herring as we hope this will work for singe loci involve in multi-locus incompatibilities
an observation that might be used to % characterize the age of hybrid zones and %YB Comment - age seems like our last paper?
identify candidate targets of selection.
...
Here we show how natural selection against hybrid incompatibilities shapes patterns of coancestry around such loci. %YB Comment. I really want people to know what we are doing by the end of the 1st paragraph
...
Our work here builds on the large body of theory describing allele frequency clines in tension zones % YB added ' allele frequency clines'
...
providing another source of information in addition to gradients in allele frequencies used in current approaches \citep[e.g.,][]{porter1997pontia, Gompert2012}. 
...
To address this idea we develop analytical theory and a set of stochastic simulations. % YB Comment. i thought a paragraph starting with neglect was a downer
Our analytical theory provides a quantitative description of the problem, but in so doing, we neglect genetic drift  \revpoint{2}{9}
...
Our simulation results  suggest that although genetic drift strongly affects local noisiness of the system,
...
\linelabel{ll:eq5tofig1} (e.g., the lower recombination event shown in Fig.~\ref{Fig:schematic}).
...
\caption{\textbf{Normalized mean enclosing block length}, $\bar l_B(m,x)$, along a simulated chromosome of length $1M$ with selected locus of $s=0.01$ at position $0.5M$ in a hybrid zone of $T=1000$. 
    ...
    The same simulation and statistic are shown in Fig.~\ref{Fig:blockLengths}, on a coarser genomic scale.
...
involve identifying alleles that are exceptional in terms of frequency across space, or genome-wide admixture proportion \citep{porter1997pontia,Gompert2012}.  
...
Our results suggest that the statistic $l_B(x,m)$ 
-- the mean genetic length of ancestry surrounding a site of a given ancestry --
could help identify selected loci in hybrid zones. 
The ratio of mean adjacent block lengths, $C(x,m)$, also shows a very sharp peak at the selected site,
...
It could be particularly important for applications to understand to what degree drift within an old, stable hybrid zone
can mimic the haplotype patterns seen in a recently formed zone, \linelabel{ll:old_drift}
in the same way that drift can produce clines at neutral loci in old hybrid zones.
...
Work on these ``islands of divergence'' \citep{turner2005genomic,Nosil2009} and related patterns have been largely descriptive \citep{Cruickshank2014,noor2009islands}.
...
%The metaphors of ``islands'', ``continents'' and ``archipelagoes'' of divergence \citep{turner2005genomic, Nosil2009} have inspired much research (and controversy  \citep{noor2009islands,Cruickshank2014}) in speciation genomics, but have been largely descriptive.  
...
our model provides some rough expectation on how quickly this should happen. 
...
We thank Graham Coop, Nancy Chen, Emily Josephs, Kristin Lee, Simon Aeschbacher, Jeremy Berg, Vince Buffalo, Nick Barton and two anonymous reviewers for helpful feedback and suggestions. This work was supported by National Institute of Food and Agriculture (NIFA) project \#1007008 to YB
and by National Science Foundation grant DBI-1262645 and a Sloan Research Fellowship to PR.
