...
to show how selection against hybrid genotypes impacts patterns of introgression
...
and linked clines persist over a  physical scale of $1/T$, where $T$ is the age, in generations, of the hybrid zone. 
...
Both reviewers appreciate the paper, but also make several criticisms.  I am considerably more positive, and believe that those criticisms that are justified can be dealt with quite easily (see below).  If the revision can be returned (very) quickly, it might be possible to still make the special issue, but otherwise, the paper would appear separately.
...
Fig 1 caption - "alelle"
\alisa{fixed}

116 - "s and sigma are small" - but relative to what?  This is ot at all obvious.
\alisa{I think this is relative to $N$ and $\tau$ respectively?}

156 - Surely if time is rescaled as wel as distance and genome length, everything should look the same?  All the results, and to a good approximation the simulations, are in the 
continuous regime where we can rescale.
...
240 - This is a discrete deme model, so dispersal cannt follow a continuouus Gaussian. I guess what is happening is that sigma is chosen larger relative to deme spacing.  However, there is some approximation here, and the approach will fail if sigma is small relatve to deme spacing.
...
495 - "reality" means "simulation" here??
...
"Although our assumed scenario of selection against heterozygotes at a single locus is uncommon in nature, we believe that this is unlikely to drastically influence our findings, as previous studies have demonstrated that underdominant loci share similar properties with more realistic models"
...
\alisa{I have no good comment for this right now, but am trying to edit the manuscript to address this.}
...
2-
Eq5:
If $a<b<\theta<0$, then I interpret $gtheta,b) = 0$. If correct, this should be specified.  Alternately, the bounds of the integral could be modified to account for this situation.
\alisa{I don't think that this is true? I think that the situation described here is accounted for by the other terms?}
...
"In the notation above, $qz(x, t, r) = Px{Zt = A}$. "
I did not see the notation $P^x$ above, and the RHS does not depend on small z.
\alisa{(This is at l180) Should the $=$ be a $:=$}

l.120 As we show below the solutions provide an good approximation
\alisa{fixed}

l.144: It seems like the x+r and x-r should have the same sign.
\alisa{I think may be correct?}
l 199:

$g_z(x, T, a,b)$ is defined as: " We are interested in the probability gz(x, t; a, b) of finding an entire segment (a, b) of ancestry A in an individual having selected allele of type z, ... "

This definition does not make it clear that the probability is conditional on z; P(A|z) vs P(A,z).

\alisa{I dont't actually agree with this... thoughts?}

l.606: Reference issues
...
By selecting ?Yes? you are opting in to the Publons service and data about this review (including your name and the review itself) will be transferred to Publons. You may opt-out of the service at any time. : No
...
Figure 1: Plotting the haplotypes to the side of the plot would help.
\alisa{We have remade Figure 1, with additional information that we hope will help with interpretation}

Simulations;
l. 245. As the theory is set up in the absence of genetic drift, and, correspondingly, the local population size is large, it is unclear how a deme with 500 individuals could become empty in a Wright-Fisher model.  The (narrow) width of the habitat also needs justification.
...
Results
l. 299 Total population size is irrelevant when predicting the effects of genetic drift (besides, some simulations only have 50 demes across, not 100). In a one-dimensional habitat, one uses N sigma sqrt(s) to judge whether the effect of genetic drift is likely to be negligible.
...
Scaling arguments, p. 14, 15: Please give a citation with the known scaling, and take more care with the novel ones.  l.339: I think the sentence about the maximum length of the enclosing haplotype, sigma sqrt(s) / x is mis-phrased: presumably this should be a statement on the distribution (expected size?) of the introgressing block length at distance x, as a lucky block can travel many sigmas in one generation?
...
p.27 rates, e.g. Eq. (7,8 ): I find the explanation of the rates a bit unclear. I understand that the recombinant AB can arise either by A reproducing at rate $s_A$ and recombining with B, or B reproducing at rate $s_B$ and recombining with A ? such that the neutral locus of the type A is kept. Or alternatively, the BA recombinant arises, where the neutral locus at B is kept. Which one of the two happens has a probability of 1/2.
\alisa{We'll work on making this more understandable.}

The "" Data Accessibility section" is missing. The folder at ....  appears to be a working folder with lots of other files and the r-scripts hidden under a ?sim? directory. The README file appears insufficient to allow orientation in the scripts, and worryingly, it has an ?oh, wrong recombination distance? note to it.
\alisa{Ok. This probably needs to be fixed.}

The Acknowledgements section is missing.
\alisa{I had noticed this. This will be added.}


Additional Questions:
Do you want to get credit for reviewing this manuscript on Publons? By selecting ?Yes? you are opting in to the Publons service and data about this review (including your name and the review itself) will be transferred to Publons. You may opt-out of the service at any time. : Yes
...
and \alisa{ancestry} haplotypes can potentially carry substantial information about the formation of recent zones.
...
% equilibrium states of clines at selected loci \citep{Nagylaki1975,barton1979dynamics,barton1986barrier,Slatkin1973,Slatkin1975,May1975,Haldane1948}.  
...
as many ancestors of the locally common type as do neutral haplotypes. \alisa{This reasoning is applicable to both instances of under dominance and ecological (e.g. local adaptation) }
...
%%%%%%%%
\begin{figure}
\includegraphics[clip,width=\textwidth,trim=0cm 0cm 0cm 0.05cm]{figs/BM_schematic_new2.pdf}
\caption{
...
and analytics, mostly in terms of partial differential equations needing numerical solution. \alisa{For simplicity, we ignore coalescence.}
...
affecting neighboring loci as well \citep{barton1979gene, Barton1986}.
...
(iii) $s$ and $\sigma$ are small, \alisa{relative to $N$ and $\tau$ respectively?}
and (iv) population density is large,
...
\plr{change $\dot p$ to $\frac{d}{dt}p$, and $\Delta$ to the second derivative?}
This says that the local rate of change of the frequency of $A$ ($\dot p$)
...
Note that rescaling space and time by $\sigma/\sqrt{s}$ and $1/s$ respectively in equation \eqref{eqn:cline_pde} 
results in a dimensionless equation \citep{Slatkin1973},
...
and thus away from the hybrid zone, but we assume that $s$ is small and ignore this.) \alisa{could this reference to small $s$ be confusing?}
...
(i.e., it moves at speed $\sigma^2$ in the potential $-\log p$). \alisa{What does potential here mean?}
...
Using the fact that $\E[R]=0$ and $\E[R^2]=\sigma^2$ \alisa{perhaps a brief explanation of how this works}
...
and the second is the mean displacement, which moves the lineage ``downhill'' on either $-\log p$ or $-\log (1-p)$
($\grad$ is the gradient),
depending on which selected allele the lineage is linked to.
The second and third expressions say that a lineage switches from an $A$ to $B$ background
at rate $r$ multiplied by the local proportion of $B$ alleles, which is $(1-p)$.
...
and where $\grad$ is the gradient, and $\Delta$ is the Laplacian \alisa{describing the rate of change}. 
...
To determine how the cline at a linked locus is expected to relax, \alisa{and to confirm the behavior of our model to previously established results,}
...
in heterozygotes, which switch the identity of the linked, selected locus
...
(\alisa{to consider cases where the selected locus lies outside the interval [a,b], i.e.,} we say that the integral $\int_a^{\min(0,b)}$ is zero if  $a>0$ 
...
This implies the enclosing $A$ haplotype surrounding the locus should be no longer than $\sigma\sqrt{s}/x$.
