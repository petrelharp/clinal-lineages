...
\begin{align} \label{eqn:cline_pde}
    \frac{d}{dt} p = \frac{\sigma^2}{2} \Delta p +  2 p (1-p)  \times s (\frac{p}{2} - 1), %YB Rearranged this equation to allow for cleaner description
...
(This sort of rescaling is common in the literature, e.g., \citep{fisher1950frequencies,Slatkin1973}.)  \linelabel{ll:rescaling}
...
so this equivalently takes a centered Gaussian displacement and assigns it to the nearest deme. \revpoint{AE}{5}
...
stochasticity has substantially moved allele frequencies range-wide, \yb{which is in accordance with previous theory showing that tension zones wobble at a rate proportional $\sqrt{T}$ \citep{Barton1979a}. }
...
Power will be optimal at intermediate values of $s$ \alisa{relative to $\sigma$}.
If selection is too strong, it may be difficult to observe this signal
due to a lack of introgressed selected sites,
while if selection is weak, the selected lineage does not move very fast \alisa{back in time},
...
as previous studies have demonstrated that models of underdominant selection 
share similar properties with more realistic models (e.g., ecological selection \citep{Barton1989,Barton1993}). 
\revpoint{1}{1}
\plr{(more detail here or reference intro. would like to say `indistinguishable'; who can we cite?)}
