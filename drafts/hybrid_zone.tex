...
and linked clines persist over a  genomic scale of $1/T$, where $T$ is the age, in generations, of the hybrid zone. \alisa{This is in May1975? Should it be removed?}
...
creates a gradient of alleles derived from the two populations across geographic space, %YB removed `assignments' as it seems like it is the ancestry, not its assignment, that is of interest
...
These patterns of correlated ancestry in admixed populations have been previously used to infer histories of hybridization and admixture \citep[e.g.][]{Gravel2012,Hellenthal2014,Harris2013,sedghifar2015spatial}.  

%The mixing of genomes across space, combined with selection, results in the formation of genetic clines. 
%These patterns have been the traditional focus of both empirical and theoretical studies into hybrid zones. 
%While here we focus on hybrid zones maintained by selection against hybrids 
%-- \emph{tension zones}, as reviewed in \citet{Barton1985} --  % removed \citet{Bazykin1969}, it's cited later
%clines may also be due to differential selection across environmental gradients \citep{Haldane1948,fisher1950frequencies,hanson1966effects},
%% generating \emph{ecotones}.  % I think ecotone means something somewhat different: see Barton & Hewitt 1985, p. 132, for instance
%Although these are in principle different,  \linelabel{ll:all_clines_are_the_same}
%theoretical and simulation studies show that
%genomic clines seen in the two situations are effectively indistinguishable without additional information \citep{may1975gene,Barton1993,Kruuk1999},
%with their shape determined mostly by the mean fitness of the heterozygote \citep{Slatkin1973}.
% consistently show ecotones and tension zones have very similar properties \citep{Slatkin1973, barton1986barrier,Kruuk1999}, 
% \plr{(I can't find discussion of similarity of different types of clines in \citet{barton1986barrier}: put the citation back in if I missed it.)}
%% don't think we need this next bit
% The primary difference being that the center of a tension zone fluctuates over time \citep{barton1979dynamics} 
% because stochastic changes in allele frequency can shit the cline center, 
% while centers of ecotones do not change unless the environmental gradient changes. 


Our work here builds on the large body of theory describing tension zones \alisa{and aims to describe \emph{transient} dynamics with a focus on \emph{haplotype} patterns.}
%While much of this previous work describes equilibrium states of genetic clines, 
%we focus on \emph{transients}, in particular describing haplotype patterns. 
%Although the cline in selected allele frequency establishes quickly, transient patterns can persist for quite some time, 
...
understanding the temporal dynamics of the system by describing properties of the lineages tracing back from the zone.
% This is particularly suited to understanding transient patterns.  


The arrangement of segments of genome inherited from particular ancestral populations%, which may be identifiable as distinct haplotypes,
is shaped by the number of recombination events that have taken place between ancestral genotypes. 
The length distribution of such segments therefore contains information about the age of the hybrid zone and the relative strength of selection affecting subsets of the genome. 
...
\yb{We show here that heterogeneous patterns of the ancestry length distribution across the genome may help identify putative targets of selection in hybridizing populations, providing another source of information in addition to gradients in allele frequencies  used in current approaches \citep[e.g.,][]{Porter1997, Gompert2012}. Specifically, we find that selection against hybrid incompatibility loci changes the extent of correlated ancestry --- that is, 
...
\alisa{We extend the framework presented by \cite{sedghifar2015spatial} to describe how lineages move across a zone of secondary contact. We consider the influence of selection against heterozygotes at a single locus on the movement of lineages across space, conditioning on the frequencies of the selected allele and allowing lineages to recombine between selected background } (analogous to the structured coalescent applied to selection, as in \citet{hudson1988coalescent}).
...
as many ancestors of the locally common type as do neutral haplotypes. \alisa{This reasoning is applicable to instances of both tension zones generated by intrinsic genetic incompatibilities, and ecotone models of extrinsic ecological selection (e.g., local adaptation).}
%\plr{(ecological what?)}

\yb{Because of the well-known problems with spatial coalescence \citep{felsenstein1975pain,barton2002continuous},
incorporating genetic drift is not straightforward, and we therefore neglect genetic drift in our analytical results.} \revpoint{2}{9}
This approach is common to nearly all theoretical work,
that study the reaction-diffusion equations that govern the deterministic, high-density limit \citep[as in][]{Nagylaki1975},
or the discrete analogues \citep{hanson1966effects}. 
There is substantial work on how clines are affected by genetic drift \citep{Slatkin1975,felsenstein1975genetic,Nagylaki1978,durrett2007width,barton2008effect,Polechova2011},
and quantifying the barrier to gene flow caused by clines \citep{barton1979gene,Barton1989,Barton1993,Barton2000}.
\plr{(why are references to 'barrier to gene flow' in the paragraph on drift? perhaps move to discussion?)}
While we do not attempt to account for finite population size in our analytic results,
our simulation results (below) suggest that although genetic drift strongly affects local noisiness of the system,
it does not substantially influence our conclusions at moderate population densities
(hundreds within one dispersal distance),
perhaps because of the short time-scales that we examine.
\plr{(maybe remove this next sentence? but want to say something about the very low densities we don't examine.)}
At much lower population densities, local extinction and recolonization are expected to have a stronger effect.

\plr{(nice wrap-up sentence shouldn't be at the end of paragraph on drift:)} \alisa{Is it weird to end intro on above paragraph? Could this wrap-up sentence hold up on its own? }
\alisa{Our results look beyond the traditional signature of selection observed in the width of genetic clines, and present a possible new approach for identifying genomic regions experiencing selection in hybrid zones.}
...
\yb{because selection on an allele depends on its marginal effect on fitness.  }
...
Likewise, while we do not model ecological selection, theoretical and simulation studies show that clines formed by underdominant selection and extrinsic selection are effectively indistinguishable without additional information \citep{may1975gene,Barton1993,Kruuk1999}, especially over short time scales \citep{Barton1979a}, \linelabel{ll:all_clines_are_the_same}
with their shape determined mostly by the mean fitness of the heterozygote \citep{Slatkin1973}. 
...
\caption{
    ...
    The \alisa{rightmost} blue line represents the path of the selected locus. 
   % \plr{(All lines look the same width: no thick and thin.)}
...
\plr{As noted above, we ignore the effects of drift and the pedigree,
so that the underlying allele frequency dynamics are deterministic.
This should provide a good model for stochastic motion of a single lineage (our main focus),
but does not model correlations between lineages.}
...
\begin{align} \label{eqn:cline_pde}
    \frac{d}{dt} p = \frac{\sigma^2}{2} \frac{d^2}{dx^2} p +  2 p (1-p)  \times s (\frac{p}{2} - 1), %YB Rearranged this equation to allow for cleaner description
    %    \frac{d}{dt} p = \frac{\sigma^2}{2} \frac{d}{dx^2} p + s p (1-p) (2p-1) ,
\end{align}
where $ \frac{d}{dt} p$ is the local rate of change of the frequency of $A$.
...
describes the impact of selection on local allele frequency as the product of local genotypic variance, 
$2 p (1-p)$, and selection on allele $A$, $s (p/2-1)$. 
\plr{ I don't think 'genotypic variance' is the right interpretation?  Let me try again: 
The second term is the product of the local frequency of heterozygotes, $2p(1-p)$,
and the strength of selection on allele $A$, $s (p/2-1)$.  }
...
\yb{We note that because recombination occurs within a deme, the recombination terms in equation \eqref{eqn:q_pde} are quite standard \citep{HartlClark}.} \revpoint{1}{4}
...
so define these as
$G^A = \sigma^2 \frac{d}{dx} \log(p(x,t)) \cdot \frac{d}{dx} + (\sigma^2/2) \frac{d}{dx^2}$
and
$G^B = \sigma^2 \frac{d}{dx} \log(1-p(x,t)) \cdot \frac{d}{dx} + (\sigma^2/2) \frac{d}{dx^2}$,
...
This allows for numerical solutions, %`efficient` may be an overstatement
...
% removed "ancestor's genotype" because it's not really genotype
...
\includegraphics{figs/alleleFrequencies_sim_comparison_revision_original.pdf}
    \caption{
        ...
        and $r=0$ represents the locus that is under selection. Colors correspond to different values of $r$, transitioning from red (tight linkage to selected site) to blue (distantly linked).
...
\plr{Below we use $l_B$ to mean the normalized version also.  I am defining consistent notation, $\bar l_B$.}
...
Figure \ref{fig:alleleFreq_tau100_comparison} shows that expected clines computed from equation \eqref{eqn:q_pde}
...
and so the strength of signal from elevated $l_B(m)$ is diminished \alisa{(Fig.~\ref{sfig:blockLengthPlot_t1000_s0.001})}. 
...
we believe that this is unlikely to drastically influence our findings.
As discussed in the Introduction,
previous studies have demonstrated that models of underdominant selection 
share similar properties with more realistic models, e.g.,
selection on an ecological gradient or multilocus, epistatic selection
\citep{Slatkin1973,Barton1993,Kruuk1999}.
\revpoint{1}{1}
