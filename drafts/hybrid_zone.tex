...
Within a genome, the mixing of ancestries is brought about by the the process of recombination that takes place every generation \yb{(CITE Fisher1954, Chapman \& Thomson; Barid et al)}.   
...
If hybrids have non-zero fitness, and some amount of introgression can occur, and patterns of introgression can provide information about the strength of selection \yb{in addition to the} extent of hybridization, and timing of secondary contact \yb{(CITE: hapmix, Hellenthal2014)}. 
...
We consider two isolated populations --- labeled \emph{Species $A$} and \emph{Species $B$} ---  come into contact $\tau$ generations in the past. 
...
\yb{Although most selection in hybrid zones is likely more complex than the single locus intrinsic under-dominance we consider, previous work has shown that such models generate clines that are effectively indistinguishable from more complex models of ecological selection (CITE: (BARTON and HEWITT 1989; BARTON and GALE 1993) which also generate clines like those degenerated under complex epistatic systems}. Specifically, here the fitness of an individual is  determined by its genotype at a particular locus $L$, such that individuals that are heterozygous at locus $L$ have fitness reduced by $s>0$ compared to homozygotes. To fix notation, we will say that species $A$ was initially on the ``left'' of the zone of contact, which corresponds to spatial positions $x<0$. 
...
Most attempts to identify loci under selection in hybrid zones do so by identifying exception allele frequencies as a function of geographic position or genome-wide admixture proportion \yb{(CITE THINGS)}.  
Current genomic approaches to the study of hybrid zones provide a additional rich sources of information -- namely ancestry deconvolution facilitated by programs such as hapMIX, LAMP and fineSTRUCTURE \yb{(CITE)} provides researchers with a measure of the length of ancestry tracts across the genome. 
The information contained in the length distribution of these ancestry tracts genome-wide  has been extensively used to infer the demographic history of hybridization and admixture.   
However, how selection influences the length of tracts of ancestry around selected sites in hybrid zones has received much less attention. 
We we present an expectation of ancestry block lengths in hybrid zone. 
We find that selection against hybrid incompatibilities leaves a distinct genomic signature in the form of long contiguous blocks of ancestry around these loci.   

Our finding that long blocks of ancestry surround selected sites in hybrid zones reflects the intuition that regions surrounding selected loci are going to have a harder time introgressing. 
Thus we can interpret this pattern as Brownian motion with drift in which ancestry which finds itself on the wrong side of a hybrid zone rapidly moves, backwards in time, across space to the "correct" side of the hybrid zone.  
Selected alleles that have, by chance, encroached deep into the other side of the hybrid have likely to have done so quite recently, as they cannot survive long in such hostile territory.   
...
We further found that the long blocks of ancestry around selected sites on the wrong side of the hybrid zone will likely come from a large number of independent ancestors   (Figure~\ref{Fig:family_size}). % is this right 
This result also reflects the recency of ancestry on the "wrong" side of the hybrid zone --- any one lineage will rapidly retreat back in time to the correct side of the zone.   
Thus we predict 
...
These two novel finding may be beyond detection in current research programs that often make use of low-density genetic markers for which phase is unknown. 
However, we as sequencing prices continue to drop and long phased reads become more common \yb{(CITE doi:10.1038/nbt0515-433 )} %Startups use short-read data to expand long-read sequencing market Michael Eisenstein Nature Biotechnology 33, 433?435 (2015) doi:10.1038/nbt0515-433 Published online 12 May 2015
ressarchers will soon have access to information pertaining to both of these findings and thus these results  could further aid in identifying selected sites in hybrid zones. 
...
This statistic, perhaps in relation to the length of ancestry around adjacent sites, could be developed into an approach to stand genomes from hybrid zones to identify putatively selected loci. 

\subsection{Assumptions}
We made numerous simplifying assumptions to derive our analytical and simulation results. 
Thus before our results can be profitably used for inference, it is worthwhile to  reflect on our assumptions and their consequences. 
...
However, other assumptions of our model may be of greater consequence. 
Importantly, our work considered only a single selected site.   
While a small number of selected sites will likely have little influence on our results, a higher density of selected sites could. 
Specifically, numerous selected sites within one ancestry block would likely act to both  increase the strength of selection, and to generate a longer unit which cannot be effectively broken up by recombination   \yb{(CITE: (BARTON 1983; Kruuk 1989)}. 
Such regions would likely be surrounded by even greater chunks of unbroken ancestry -- a result reflecting this great $s$ and the map distance between these linked incompatibilities. 

Another important assumption is the smooth and one dimensional spatial structure of the geographic landscape.  
Patchily connected populations, or other esoteric forms of population structure may predominate on the edges of species ranges' where hybrid zones may occur.  
While, it is still likely that the genomic signature of longer chunks of ancestry around selected sites will hold in these situations, the strength of this expectation is unclear.     
...
The past years have witnessed evocative metaphors f seascapes to describe heterogeneous patterns of genetic divergence across the genome.  
