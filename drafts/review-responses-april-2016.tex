...
Thus, while some results are fairly standard, they might not be familiar to our readership.  
One of our concerns is that the math would be largely overlooked if moved into the supplement (which is not automatically printed with the main document) and this could create a barrier to understanding among the readers.}
...
\reply
\alisa{This is a point we have considered ourselves. We present these equations as a way of building our model up to address haplotype lengths. Both analytical expressions and simulations are presented so that they each support the conclusions of the other.} \yb{However, we also believe that analytical results can allow for a deeper understanding than simulation output. 
...
\reply
\yb{We agree that a long set of theoretical results can be tedious. We have therefore reshaped the introduction to highlight our major question ``How does selection against incompatibilities in a hybrid zone influence the length distribution of ancestry tracts'' and result ``'It shortens them''.}
...
\reply
\yb{We agree that this representation has likely been derived many times. We turned to Hartl and Clark and found a similar derivation. We therefore signpost that this is a standard result by pointing readers to that text.}   
...
\reply
\yb{Thanks for spotting this. Because we do nothing quantitative with respect to drift here, we simply state the our observed wobble of the center of the tension zone is consistent with previous theory. However, more broadly throughout the manuscript we are not sure about  relying on $N \sigma \sqrt{s}$ as we are concerned with loci linked to the selected site, and we are not aware of results extending this rule to an unlinked locus.}
