...
We are pleased to submit a revision of our Manuscript, `Beyond clines: lineages and haplotype blocks in hybrid zones' (MEC-16-0266) for the special issue 'Genomics of Hybridization'. 
We believe that this version is much improved by changes we have made to address reviewer and AE comments. 
Below we provide a point-by-point response to reviewer and AE comments, describing how we addressed concerns raised by our readers. 

The major themes of our changes are 
(\textbf{1}) Attempting to make our mathematical derivations more accessible for Molecular Ecology's wide audience  
(\textbf{2}) Additional justification for using a genetic model of underdominance to model hybrid incompatibilities, in which we make heavy reference to the historical theory justifying this approach, 
(\textbf{3}) Additional simulations to show that our results are not driven by boundaries imposed in a defined range, and 
(\textbf{4})  A streamlined introduction that makes clear our major goals and provides a stronger structure for our manuscript. 

Thanks to both reviewers and the AE for the useful (and prompt) feedback.
...
We would also like very much to develop a practical test,
but agree that falls outside of the scope of this paper.
...
we now provide citations to the literature showing that different underlying mechanims give similar clines,
and have put in some citations to the literature making similar arguments (which we did indeed think of as standard).
...
We have added more discussion in this direction, as described below.
...
we've inserted comments to this effect in several places, including at \llname{ll:rescaling} and \llname{ll:may_citation}.
...
\reply
Figure 1 has been remade with additional information and details that we hope will aid in the understanding of our framework. We have related the recombination events in the figure to the terms in Equation 5. \llname{ll:eq5tofig1}
...
and a note about drift maintaining certain clines in old zones at \llname{ll:old_drift}.
