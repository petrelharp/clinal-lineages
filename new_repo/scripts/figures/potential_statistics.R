...
load("simulation_SIGMA1_Ninds25000_ndemes50_s0.001_tau1000_run2016-02-01_simsums.Robj")
#load("simulation_SIGMA1_Ninds25000_ndemes50_s0_tau1000_simsums.Robj")
#load("simulation_SIGMA1_Ninds25000_ndemes50_s0.01_tau200_run2016-01-28_simsums.Robj")
...
get.interval.size.anyanc= function(IND_DATA=sims.sums[[1]]$ind.ancest[[1]],CHR=chromsoome,POS=0.5,ancB=TRUE){
	chunk = sapply(IND_DATA[[CHR]],function(X){diff(as.numeric(X[which(X$starts<POS & X$stops>POS),1:2]))})	
}

intervalSizes_allAncs = lapply(positions,function(POS){
				do.call(rbind,lapply(sims.sums[[1]]$ind.ancest,function(IND){
				get.interval.size.anyanc(IND_DATA=IND,CHR=chromosome,POS=POS,ancB=TRUE)	
				}))
				})
...
plot(mean_per_deme_AncB[,1]/mean(mean_per_deme_AncB[,1],na.rm=T),ylim=c(0.5,2),type="l",lty=1,col=c(rainbow(25),rev(rainbow(25))),main="AncB ancestry, normalized by deme (col reflects deme pos)",cex.main=0.8)
...
save(flanking.blocks.by.ind,file=paste(outstring,"flanking.blocks.by.ind.Robj",sep="_"))
...
save(flanking.blocks.by.ind.AncB,file=paste(outstring,"flanking.blocks.by.ind.AncB.Robj",sep="_"))
...
plot(ylim=c(0,2),xlim=range(positions),0,0,col="white",ylab="mean length",xlab="physical position",main="flanking block length| AncB, normalized deme",cex.main=0.7)
for(i in 1:50){points(flanking.blocks.deme.matrix.AncB[i,]/mean(flanking.blocks.deme.matrix.AncB[i,],na.rm=T),x=positions,type="l",col=rainbow(50)[i])}
...
image(t(flanking.blocks.deme.matrix),x=positions,y=-25:25,ylab="deme",xlab="phys.pos",main="flanking heatmap ALL")
image(t(flanking.blocks.deme.matrix.AncB),x=positions,y=-25:25,ylab="deme",xlab="phys.pos",main="flanking heatmap AncB")



image(((mean_per_deme)/t(flanking.blocks.deme.matrix)),x=positions,y=-25:25,ylab="deme",xlab="phys.pos",main="focal/flanking heatmap ALL")
image(((mean_per_deme_AncB)/t(flanking.blocks.deme.matrix.AncB)),x=positions,y=-25:25,ylab="deme",xlab="phys.pos",main="focal/flanking heatmap AncB")
